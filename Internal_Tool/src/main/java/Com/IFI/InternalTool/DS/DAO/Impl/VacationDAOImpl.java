...
public class VacationDAOImpl implements VacationDAO {
	...
	public List<Vacation> getAllVacationByEmp2(long manager_id, int page, int pageSize, String sortedColumn,
			Boolean desc) {
		Session session = entityManagerFactory.unwrap(SessionFactory.class).openSession();
		String hql = "Select v FROM Vacation v INNER JOIN Employee AS e ON v.employee_id= e.employee_id INNER JOIN Project AS p ON v.project_id=p.project_id INNER JOIN Project_Manager AS pm ON (v.employee_id=pm.employee_id and v.project_id=pm.project_id  and v.status=pm.priority)  where  pm.manager_id=:manager_id ";
		if (sortedColumn != null && desc != null) {
			String order = "";
			if (desc) {
				order = "desc";
...
//	}
	...
	public List<Long> countVacationByStatusMng(Long manager_id) {
		...
		Session session = entityManagerFactory.unwrap(SessionFactory.class).openSession();
		// need approve
		String hql1 = "Select count(*) From Vacation_Log v where v.next_approve_id=:manager_id and v.approved_id=0 and v.disapproved_id=0";
		Query query1 = session.createQuery(hql1);
		query1.setParameter("manager_id", manager_id);
		result.add((Long) query1.uniqueResult());
		// approved
		String hql2 = "Select count(*) From Vacation_Log v where v.approved_id=:manager_id";
		Query query2 = session.createQuery(hql2);
		query2.setParameter("manager_id", manager_id);
		result.add((Long) query2.uniqueResult());
		// disapproved
		String hql3 = "Select count(*) From Vacation_Log v where v.disapproved_id=:manager_id";
		Query query3 = session.createQuery(hql3);
